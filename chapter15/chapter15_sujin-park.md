...
- 잘린 메시지를 캐시하는 위험을 줄이기 위해, 캐싱 프락시 서버는 명시적으로 Content-Length 헤더를 갖고 있지 않은 HTTP 본문은 캐시하지 않습니다.
...
Content-Length 헤더 또한 인코딩된 본문의 길이를 바이트 단위로 정의합니다.
...
- HTTP 폼을 채워서 제출하면, 가변 길이 텍스트 필드와 업로드 될 객체는 각각이 멀티파트 본문을 구성하는 하나의 파트가 되어 보내집니다.
...
인코딩은 발송하는 쪽에서 콘텐츠를 적용합니다. 콘텐츠 인코딩이 끝난 데이터는 늘 그렇듯 엔터티 본문에 담아 수신자에게 보냅니다.
...
- 집합은 반드시 `chunked`를 포함해야 함. 예외는 메시지가 커넥션의 종료로 끝나는 경우
